...
void encoder_reset(encoder_t *enc, int16_t init_pos, int16_t low, int16_t upp, uint8_t inc, uint8_t fast_inc, bool looped) {
    enc->min_pos = low;
    enc->max_pos = upp;
...
void encoder_change_isr(encoder_t *enc) {
    ...
    if (!a) {
        ...
        if (enc->a_low_start_time > 0) {
            uint8_t inc = enc->increment;
            if ((now - enc->a_low_start_time) < ENC_OVER_PRESS) {
                if ((now - enc->last_change_time) < ENC_FAST_TIMEOUT) {
                    inc = enc->fast_increment;
                }
